        ...
        },
        {
            $set:
            {
                char:
                {
                    limbs:
                    {
                        head: head, body: body, rightArm: right_arm, leftArm: left_arm, rightLeg: right_leg, leftLeg: left_leg
                    }
                }
            }
        }
...
const customMonster = (req, res) => {
    if (req.user && req.user.char.limbs.head) {
        ...
        res.render("home", {
            userStatus: req.user ? true : false, user: { head: 'zombo_head', body: 'zombo_body', rightArm: 'zombo_right_arm', leftArm: 'zombo_left_arm', rightLeg: 'zombo_right_leg', leftLeg: 'zombo_left_leg' },
